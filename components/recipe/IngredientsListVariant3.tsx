      ...
      {ingredients.map((ingredient, index) => (
        ...
        >
          <div className="flex items-start gap-4">
            {/* Icon Circle */}
            <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-accent/10 flex items-center justify-center border-2 border-accent/20">
              <svg
            ...
            {/* Decorative element */}
            <div className="absolute top-0 right-0 w-20 h-20 bg-accent/5 rounded-bl-full transform translate-x-8 -translate-y-8 group-hover:translate-x-6 group-hover:-translate-y-6 transition-transform duration-300" />
          </div>
        </div>
      ))}
    </div>
